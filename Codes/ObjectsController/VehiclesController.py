from Codes.Configuration import Vehicle_characteristics, HIGH_NUMBER_OF_VEHICLE, \
    LOW_NUMBER_OF_VEHICLE
...
class VehicleController:
    ...
    def add_vehicles(self, routes, num_veh):
        for index in range(num_veh):
            route_id = str(uuid4())
            vehicle_id = 'Veh'+str(uuid4())
            route.add(routeID=route_id, edges=routes)
            vehicle.add(vehID=vehicle_id, routeID=route_id)
            vehicle.setLength(vehicle_id, Vehicle_characteristics['length'])
            vehicle.setMinGap(vehicle_id, Vehicle_characteristics['min_cap'])
    ...
    def generate_vehicles(self, sub_episode_number, step):
        for _, c_edge in enumerate(self.incomming_edges):
            for _, nc_edge in enumerate(self.outcomming_edges):
